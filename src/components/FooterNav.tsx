          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.path}
                className={cn(
                  "flex flex-col items-center gap-0.5 sm:gap-1 px-2 sm:px-4 py-1.5 sm:py-2 rounded-xl transition-all duration-300 group relative",
                  isActive
                    ? "text-primary"
              ...
              >
                {isActive && (
                  <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-8 sm:w-12 h-0.5 sm:h-1 bg-gradient-primary rounded-full" />
                )}
                <div
                  className={cn(
